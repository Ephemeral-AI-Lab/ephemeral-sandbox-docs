...
3. `roots.catalog` generation swap after root/manifest/object durability is the sole candidate linearization point.
...
6. Recovery trusts authoritative catalogs and fences, not a journal label; pre-commit failure leaves the old head, post-commit lost response returns committed.
...
├── layer-stack/
│   ├── .storage-writer.lock
│   ├── manifest.json                                                 legacy v1 sole public publication authority
...
│   │   └── index.catalog
│   ├── catalogs/v1/
│   │   ├── roots.catalog                                           private branch heads/generations/receipts
│   │   ├── locators.catalog
│   │   ├── materializations.catalog                                 MaterializationKey=(RootId,backend_kind,backend_format_version,target_profile)
│   │   ├── leases.catalog                                          authoritative lease generation/fences
│   │   └── retention.catalog                                       reserved Stage 08
│   ├── journals/v1/
│   │   ├── publication/<PublicationId>.journal
│   │   ├── hydration/
│   │   ├── squash/                                                  reserved Stage 09
│   │   ├── compaction/                                              reserved Stage 08
│   │   └── migration/
│   ├── staging/v2/
...
| --- | --- |
| Before | Known legacy head/private branch head/generation; no run request journal, staging, or lease; workspace commands still admitted until finalize fence. |
| `Prepared` | Fsynced journal + transaction/base lease; no new object/root/head; public legacy state unchanged. Crash → fence/abort/release. |
| `ObjectsDurable` | Verified immutable objects/locators may remain; no root/head visibility. Crash → reuse safe objects, clean transaction staging. |
| `RootDurable` | Immutable manifest/root may exist but branch head remains old. Crash → request reconciliation may resume CAS; no public/candidate head guess. |
| `CommitIntent` | Expected head/root/fence durable; head still old until catalog swap. Stale/overlap → `Conflict`, staging cleanup, leases release. |
| `Committed` visibility | `roots.catalog` atomically names new root/generation and request receipt after directory fsync. Response loss/restart → exact committed receipt. |
| After commit before terminal journal | Catalog wins; recovery writes terminal `Committed/Settling`; never rolls head back. |
| Lease failpoints/restart | Catalog/lease record generation chooses the valid token; stale owner/fence cannot renew/release another lease; no deletion follows. |
...
| Cleanup | No nonterminal journal/staging/transaction lease/frozen capture; committed roots/objects/receipt and active declared leases remain; tracked sessions/config restored. |

The Rust test injects every journal fsync, object/locator, root/manifest, commit-intent, catalog temp/rename/directory-fsync, terminal receipt, lease acquire/renew/release, cancellation, and restart boundary. The packaged cases inject one pre-linearization and one post-linearization/lost-response boundary, plus lease restart.
...
| `phase1.final.publication.qualification` | final; `planned-final` | candidate publication full matrix | frozen release/scale/corpus and required runners using one pinned Ubuntu 24.04 target image | Stage 11 full qualification | all atomicity, OCC, recovery, authority, performance, space, memory, and required-runner gates | normative matched metrics | complete amplification envelope | full memory, history, soak matrix | required runners, pinned Ubuntu 24.04 target; cross-image deferred beyond Phase 1 | `300000` ms | Stage 11 qualification bundle |
...
| `phase1.final.publication.qualification` | `Final durable publication qualification` | `Executes the frozen Stage 11 correctness, atomicity, OCC, recovery, authority, performance, space, memory, soak, rollback, and required host/release-runner matrix using the one pinned Ubuntu 24.04 target image; cross-image qualification is deferred beyond Phase 1.` | `("phase1.qualification","storage.publication_v2","storage.occ","storage.recovery","portability","ubuntu-24.04")` | `{"terminal":"All final publication atomicity, OCC, retry, recovery, authority, time, amplification, memory, history, soak, and required-runner gates execute successfully against the pinned Ubuntu 24.04 target image with complete evidence; no cross-image acceptance is claimed."}` | `{"terminal":("phase1.qualification","storage.publication_v2","storage.occ","storage.recovery","observability.resource_efficiency","portability","ubuntu-24.04")}` | `"cli"` | `"phase1-storage"` | `300000` | `("release","phase1","config")` |
...
| Concurrent disjoint, overlapping conflict, stale expected generation | focused now | disjoint rebase; conflict/no lost update; generation CAS |
| Failure/crash before/after every journal/object/locator/root/head/receipt boundary | all Rust now; representative packaged | catalog-first deterministic state table in §3 |
...
| Corrupt/missing/truncated/mismatched object/root/journal/catalog | Rust exhaustive; packaged catalog/object representative if safe | typed fail closed/quarantine; public legacy unchanged |
...
| Full required release/host matrix on the sole pinned Ubuntu target, plus scale/history/soak | planned-final | Stage 11 |

Success must show both authorities: the public legacy result and the distinct private branch/catalog receipt. A legacy success without `commit_linearized`, request/branch/root/generation, OCC, and recovery evidence is not a Stage 07 pass.
...
Hard caps: loop ≤60 s; every operation ≤60 s; identity/receipt/OCC/authority exact; SeqCDC chunk range exact; publication peak ≤`C_capture+5%`; ring 32 KiB; ≤4 borrowed chunks/workers; queue 16/64 KiB; merge 8×64 KiB; encoder 256 KiB; cache 16 MiB; semaphore 64 MiB; managed ≤4 MiB/op excluding cache; RSS ≤384 MiB and idle +128 MiB; settled journal bound `≤1 MiB+min(1% retained payload,64 MiB)`. Report small-edit `baseline+15%+5 ms` and disjoint throughput ≥90% as diagnostic POC lines; normative p95/sample corpus remains Stage 11.

## 7. Memory-stability and reclamation test

Stage 07 adds chunk/object/manifest/journal buffers, external merge readers, queues, tasks/workers, permits, transaction/lease registries, frozen capture handles, and recovery ownership. Use one long-lived daemon with no restart between tiny measured iterations; restart evidence is a separate case.
...
Quiescence poll ≤5 s requires no nonterminal txn, worker/task/queue/merge reader/borrowed view/permit/frozen capture/transaction lease/extra FD, staging, or pending journal; terminal receipts and explicitly active root/carrier leases are bounded durable state; cache is within cap. Freeze control noise before candidate as `max(8 MiB,4×control MAD)`.
...
is absent, so no scalar/accelerated differential applies. Run the packaged
publish/edit through the sole pinned Ubuntu OCI index
...
Fixed-width catalog/journal/lease
...
platform manifest, with unavailable required rows `unverified`. Cross-image
...
### DO NOT RUN in Stage 07 — Stage 11 native-host/pinned-Ubuntu-24 matrix
...
| `commit_linearized` | roots.catalog generation swap durably fsynced | result + catalog snapshot | old head | one point only | blocker |
...
| `journal_recovery_bytes`, `pending_transactions` | terminal journal/recovery residue | settled snapshot | none | bound in §6; pending 0 | blocker |
...
Mandatory pass: three run-now cases; focused formatting/lint/unit/property/transaction/recovery tests; artifact/schema validation; exact external graph equality; current-host scalar/no-helper portability; and cleanup. Atomicity, authority separation, idempotency, OCC, catalog-first recovery, lease fencing, private exact comparison, logical release, physical caps, and publication peak are hard gates.

Allow only attributed allocator/page-cache retention inside the frozen band or explicitly unavailable optional split counters. Disqualifiers include candidate mutation of public head/response, competing authority, mutable capture, duplicate/different retry error, lost update, journal-trusted false commit, stale lease mutation, any deletion, partial visibility, mismatch, operation ≥60 s, resource/space cap breach, suspected/confirmed leak, missing gate data, dependency/runtime/helper delta, or incomplete artifacts/cleanup.

Cleanup is run-ID/request/branch scoped: restore config/gateway, destroy tracked sessions, recover/terminalize transactions, release test leases, remove only transaction staging, and preserve committed immutable candidate roots/objects/receipts plus all legacy state. Rebuild after product changes; otherwise rerun only the failed focused case after diagnosis. Disable the private trigger on failure. Stage 08 still owns retention/packs/GC, Stage 09 squash/remount, Stage 10 public candidate authority, and Stage 11 normative regression/performance/space/memory/portability/soak/rollback qualification.
